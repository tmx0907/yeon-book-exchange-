...
import { MapPin, MessageCircle, ArrowRightLeft, Plus, User, Trash2 } from 'lucide-react';
...
interface BookGridProps {
  ...
  isMyLibrary?: boolean;
  onAddBook?: () => void;
  onDeleteBook?: (book: Book) => void; // New prop for deleting books
          ...
          return (
            <div key={book.id} className={`group cursor-pointer ${book.status === 'Swapped' ? 'opacity-60' : ''}`}>
              <div className="relative aspect-[3/4] w-full overflow-hidden rounded-2xl bg-slate-100 mb-6 shadow-sm border border-slate-100">
                ...
                {/* Delete Button for My Library */}
                {isMyLibrary && onDeleteBook && (
                  <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        if (window.confirm(lang === 'ko' ? '이 책을 삭제하시겠습니까?' : 'Are you sure you want to delete this book?')) {
                          onDeleteBook(book);
                        }
                      }}
                      className="p-2 bg-red-500 hover:bg-red-600 text-white rounded-full shadow-lg transition-all transform hover:scale-110"
                      title={lang === 'ko' ? '책 삭제' : 'Delete book'}
                    >
